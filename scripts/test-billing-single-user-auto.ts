#!/usr/bin/env tsx
/**
 * Test script to run billing for a single user (auto-confirm version)
 * Same as test-billing-single-user.ts but without confirmation prompt
 * 
 * Run with: npx tsx scripts/test-billing-single-user-auto.ts [email]
 */
...
async function main() {
  ...
  try {
    ...
    if (couponApp) {
      ...
      if (couponApp.coupon.flatDiscount > 0) {
        console.log(`  Flat discount: ₩${couponApp.coupon.flatDiscount.toLocaleString()}`);
      }
      console.log(
    ...
    if (result.success) {
      ...
      // Check coupon application after payment
      if (couponApp) {
        const updatedCouponApp = await prisma.couponApplication.findUnique({
          where: { id: couponApp.id },
        });
        
        if (updatedCouponApp) {
          console.log(`\n📋 Coupon status after payment:`);
