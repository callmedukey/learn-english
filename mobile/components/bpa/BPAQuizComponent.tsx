...
import { QuizResult } from "@/components/novel/QuizResult";
import { QuizStartCard } from "@/components/novel/QuizStartCard";
import { QuizTimer } from "@/components/novel/QuizTimer";
import {
  useCompleteBPAQuestion,
  useCompleteBPAQuiz,
  useMarkBPAQuestionStarted,
} from "@/hooks/useBPA";
import type {
  BPAChapterQuizData,
  BPAChapterStatus,
...
interface BPAQuizComponentProps {
  chapter: BPAChapterQuizData;
  levelId: string;
  novelId: string;
  userHasPaidSubscription: boolean;
}

export function BPAQuizComponent({
  chapter,
  levelId,
  novelId,
  userHasPaidSubscription,
}: BPAQuizComponentProps) {
  ...
  const [initialStatus] = useState<BPAChapterStatus>(chapter.status);

  const markStartedMutation = useMarkBPAQuestionStarted();
  const completeQuestionMutation = useCompleteBPAQuestion();
  const completeQuizMutation = useCompleteBPAQuiz();
  ...
  // Handle app state changes (background/foreground) - auto-submit when app goes to background
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      if (
        ...
        // App going to background - auto-submit as wrong
        handleTimeUp();
      }
  ...
  }, [currentQuestion, quizStarted, novelId, levelId]);
  ...
  const handleRetry = () => {
    router.replace(`/bpa/${levelId}/${novelId}/${chapter.id}?status=retry`);
  };

  const handleNextChapter = () => {
    if (chapter.nextChapter) {
      router.replace(`/bpa/${levelId}/${novelId}/${chapter.nextChapter.id}`);
    }
  };
  ...
  const handleBackToNovel = useCallback(async () => {
    ...
    router.replace(`/bpa/${levelId}/${novelId}`);
  }, [
    quizStarted,
    currentQuestion,
    isAnswered,
