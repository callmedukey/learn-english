...
interface ChallengeConfirmationButtonProps {
  arId: string;
  arLevel: string;
  hasActiveChallenge: boolean;
  challengeNovelCount?: number;
  currentMonth: string;
  currentYear: number;
...
export function ChallengeConfirmationButton({
  arId,
  arLevel,
  hasActiveChallenge,
  challengeNovelCount = 0,
  currentMonth,
  currentYear,
  userLevelLock,
  hasPendingRequest = false,
  pendingRequestId,
}: ChallengeConfirmationButtonProps) {
  ...
  const handleConfirm = async () => {
    setIsLoading(true);
    try {
      const result = await confirmChallengeParticipation("AR", arId);
  ...
  const handleLevelChangeRequest = async () => {
    ...
    setIsLoading(true);
    try {
      await requestLevelChange("AR", userLevelLock.levelId, arId);
  ...
  const isAlreadyJoined = userLevelLock?.levelId === arId;
  const isDifferentLevel = !!(userLevelLock && userLevelLock.levelId !== arId);
            ...
            <DialogDescription asChild>
              <div className="space-y-3">
                {hasActiveChallenge ? (
                  <>
                    <p>
                      You&apos;re about to join the {currentMonth} {currentYear} medal challenge for {arLevel}.
                    </p>
                    <p className="font-semibold">
                      This challenge includes {challengeNovelCount} selected novels.
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Once confirmed, you&apos;ll be locked to this level for the entire month. 
                      You can still attempt novels from other levels for practice, but only 
                  ...
                  <>
                    <p>
                      You&apos;re about to select {arLevel} as your level for {currentMonth} {currentYear}.
                    </p>
                    <p className="text-sm text-muted-foreground">
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {hasActiveChallenge 
                ? `You've successfully joined the ${arLevel} medal challenge for ${currentMonth} ${currentYear}. Good luck!`
                : `You've successfully locked in ${arLevel} as your level for ${currentMonth} ${currentYear}.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
