...
import { BulkToggleComingSoonDialog } from "../../[id]/components/bulk-toggle-coming-soon-dialog";
import { BulkToggleHiddenDialog } from "../../[id]/components/bulk-toggle-hidden-dialog";
import DeleteNovelAlert from "../../[id]/components/delete-novel-alert";
import MoveNovelDialog from "../../[id]/components/move-novel-dialog";
import { toggleNovelLock } from "../../actions/lock-novel.actions";
import { NovelData } from "../../query/novel.query";

interface SearchNovelsTableProps {
  novels: NovelData[];
  arLevels: {
    id: string;
    level: string;
...
const SearchNovelsTable: React.FC<SearchNovelsTableProps> = ({ novels, arLevels, userRole }) => {
              ...
              return (
                <TableRow key={novel.id}>
                  ...
                  <TableCell>
                    {novel.AR ? (
                      <Link
                        href={`/admin/novels/${novel.AR.id}`}
                        className="text-blue-600 hover:text-blue-800 hover:underline"
                      >
                        {novel.AR.level}
                      </Link>
                    ) : (
                      <span className="text-gray-400">No Level</span>
                    )}
                  </TableCell>
...
export default SearchNovelsTable;
