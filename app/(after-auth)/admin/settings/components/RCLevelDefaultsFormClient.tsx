...
import { updateRCLevelDefaults } from "../actions/settings-actions";

interface RCLevelWithSettings {
  id: string;
  level: string;
...
interface RCLevelDefaultsFormClientProps {
  rcLevels: RCLevelWithSettings[];
}

export default function RCLevelDefaultsFormClient({
  rcLevels,
}: RCLevelDefaultsFormClientProps) {
  const [editingLevelId, setEditingLevelId] = useState<string | null>(null);
  const [state, formAction, isPending] = useActionState(updateRCLevelDefaults, null);
  const [levelDefaults, setLevelDefaults] = useState<{[key: string]: {timer: number, score: number}}>(() => {
    ...
    rcLevels.forEach(level => {
      defaults[level.id] = {
        timer: level.RCLevelSettings?.defaultTimer || 30,
        score: level.RCLevelSettings?.defaultScore || 100,
      };
    });
    return defaults;
  });
  ...
  const handleCancel = () => {
    ...
    const level = rcLevels.find(l => l.id === editingLevelId);
  ...
  if (!rcLevels || rcLevels.length === 0) {
    return (
      <p className="text-gray-500">No RC levels found. Please create RC levels first.</p>
    );
  }
