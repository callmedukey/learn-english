...
import { updateBPALevelDefaults } from "../actions/settings-actions";

interface BPALevelWithSettings {
  id: string;
  name: string;
...
interface BPALevelDefaultsFormClientProps {
  bpaLevels: BPALevelWithSettings[];
}

export default function BPALevelDefaultsFormClient({
  bpaLevels,
}: BPALevelDefaultsFormClientProps) {
  const [editingLevelId, setEditingLevelId] = useState<string | null>(null);
  const [state, formAction, isPending] = useActionState(updateBPALevelDefaults, null);
  const [levelDefaults, setLevelDefaults] = useState<{[key: string]: {timer: number, score: number}}>(() => {
    ...
    bpaLevels.forEach(level => {
      defaults[level.id] = {
        timer: level.bpaLevelSettings?.defaultTimer || 30,
        score: level.bpaLevelSettings?.defaultScore || 100,
      };
    });
    return defaults;
  });
  ...
  const handleCancel = () => {
    ...
    const level = bpaLevels.find(l => l.id === editingLevelId);
  ...
  if (!bpaLevels || bpaLevels.length === 0) {
    return (
      <p className="text-gray-500">No BPA levels found. Please create BPA levels first.</p>
    );
  }
